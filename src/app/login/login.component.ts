...
export class LoginComponent implements OnInit {
      ...
      (response: any) => {
        ...
        if (response.success) {
          ...
          if (response.principalCompanies) {
            ...
            this.companyService.setPrincipalCompanies(
              response.principalCompanies
            );
  ...
  openCompanyPeriodDialog() {
    this.dialogService
      .open(SelectCompanyPeriodDialogComponent, {
      ...
      .onClose.subscribe((selectedData) => {
        if (selectedData) {
          ...
          const foundCompany = this.companyService.principalCompanies.find(
            (c) => c.id === selectedData.companyId
          );
          if (foundCompany) {
            this.companyService
              .selectAndLoadCompany(foundCompany)
              .then((logoUrl) => console.log("Logo cargado:", logoUrl));
          }

          // Asignar el período en PeriodService
          const foundPeriod = this.periodService.periodTypes.find(
            (p) => p.id === selectedData.periodId
          );
          if (foundPeriod) {

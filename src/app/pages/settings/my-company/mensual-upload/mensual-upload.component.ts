...
export class MensualUploadComponent implements OnInit {
  ...
  tareas: Tarea[] = [
    ...
    {
      id: 5,
      nombre: "Declaración y Acuse De ISR",
      enero: false,
      febrero: false,
      marzo: false,
      abril: false,
    ...
    {
      id: 6,
      nombre: "Declaración y Acuse De IVA",
      enero: false,
      febrero: false,
      marzo: false,
      abril: false,
    ...
    {
      id: 8,
      nombre: "Opinión Cumplimiento SAT",
      enero: false,
      febrero: false,
      marzo: false,
      abril: false,
    ...
    {
      id: 9,
      nombre: "Opinión cumplimiento IMSS",
      enero: false,
      febrero: false,
      marzo: false,
      abril: false,
    ...
    {
      id: 10,
      nombre: "Opinión de cumplimiento INFONAVIT",
      enero: false,
      febrero: false,
      marzo: false,
      abril: false,
  ...
  getComment(tarea: Tarea, month: string, year: number): string {
    const doc = tarea.documents?.find(
  ...
  async processOCR(file: File): Promise<string> {
    ...
    if (isPDF) {
      try {
        ...
        console.warn(
          "⚠️ No se encontró el RFC después de la sección esperada."
        );
        return "";
      } catch (error) {
  ...
  get archivosNoCargados(): number {
    ...
    this.tareas.forEach((t) => {
      if (
        ...
        !t.documents.some(
          (doc) =>
            doc.month === this.selectedMonth && doc.year === this.selectedYear
        )
      ) {
        count++;
      }
  ...
  async uploadFile(tarea: Tarea, file: File) {
    ...
    this.http
      .post(`${environment.apiBaseUrl}/documentUpload.php`, formData)
      .pipe(
        finalize(() => {
          this.cargando = false;
  ...
  updateCounters() {
    ...
    this.incompletos = this.tareas.filter(
      (t) => t.estado === "rechazado"
    ).length;
    this.noCargados =
      this.tareas.length - this.cargados - this.completos - this.incompletos;
  ...
  descargarArchivo(tarea: Tarea, month: string, year: number) {
    ...
    if (doc && doc.file_path) {
      ...
      console.error(
        "No se encontró un archivo para descargar en el mes y año seleccionados."
      );
    }
  }
  ...
  hasDocumentForYear(tarea: Tarea, month: string, year: number): boolean {
    return (
      tarea.documents?.some((d) => d.month === month && d.year === year) ??
      false
    );
  }
  ...
  obtenerEstadoArchivos() {
    ...
    this.http
      .get<any[]>(
        `${environment.apiBaseUrl}/getDocumentStatus.php?companyId=${companyId}`
      )
      .pipe(
        finalize(() => {
          this.cargando = false;
          ...
          if (Array.isArray(response)) {
            ...
            response.forEach((doc) => {
              const tarea = this.tareas.find(
                (t) => t.id === parseInt(doc.tarea_id, 10)
              );
              if (tarea) {

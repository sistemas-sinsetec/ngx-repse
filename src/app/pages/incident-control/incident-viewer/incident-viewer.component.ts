...
export class IncidentViewerComponent implements OnInit {
  ...
  async loadWeeks() {
    ...
    if (!selectedPeriod) {
      ...
      this.toastrService.showError(
        "Por favor selecciona un periodo antes de continuar.",
        "danger"
      );
      return;
    }
      ...
      .subscribe(
        (data: any) => {
          ...
          if (this.weeks.length > 0) {
            ...
            this.toastrService.showError(
              "No hay semanas disponibles para este periodo.",
              "error"
            );
          }
          loading.dismiss();
        ...
        (error) => {
          ...
          this.toastrService.showError(
            "Error al cargar las semanas. Por favor intenta de nuevo.",
            "error"
          );
          loading.dismiss();
        }
  ...
  onWeekChange(week: any) {
    if (!week) {
      ...
      this.toastrService.showError(
        "Por favor selecciona una semana válida.",
        "error"
      );
      return;
    }
  ...
  generateDiasSemana(startDate: string, endDate: string) {
    ...
    if (!start.isValid() || !end.isValid()) {
      console.error(
        "Las fechas de inicio o fin no son válidas:",
        startDate,
        endDate
      );
      this.diasSemana = [];
  ...
  getShiftInfo(shift_id: number): Promise<any> {
    return this.http
      .get(
        `${environment.apiBaseUrl}/get_shifts.php?company_id=${this.companyId}`
      )
      .toPromise()
      .then((shifts: any[]) => {
        // Ajusta la propiedad 'shift_id' según la respuesta real del endpoint
        return shifts.find(
          (shift) => Number(shift.shift_id) === Number(shift_id)
        );
  ...
  async loadEmployees() {
    ...
    const checkCompletion = () => {
      ...
      if (requestsFinished === 2) {
        if (
          this.assignedEmployees.length === 0 &&
          this.unassignedEmployees.length === 0
        ) {
          this.toastrService.showWarning(
            "No se encontraron empleados para la fecha seleccionada.",
            "Advertencia"
          );
        }
        loading.dismiss();
    ...
    this.http
      .get(
        `${environment.apiBaseUrl}/get_assigned_employees1.php?start_date=${start_date}&end_date=${end_date}&company_id=${this.companyId}&project_id=0&week_number=${week_number}&day_of_week=${day_of_week}&user_id=${user_id}`
      )
      .subscribe({
        next: (data: any) => {
          if (Array.isArray(data)) {
            ...
            this.filteredAssignedEmployees = [];
            console.warn(
              "La respuesta de empleados asignados no es un arreglo:",
              data
            );
          }
          checkCompletion();
    ...
    this.http
      .get(
        `${environment.apiBaseUrl}/get_unassigned_employees.php?company_id=${this.companyId}&start_date=${start_date}&end_date=${end_date}&week_number=${week_number}&day_of_week=${day_of_week}&user_id=${user_id}`
      )
      .subscribe({
        next: (data: any) => {
          if (Array.isArray(data)) {
            ...
            this.filteredUnassignedEmployees = [];
            console.warn(
              "La respuesta de empleados no asignados no es un arreglo:",
              data
            );
          }
          checkCompletion();
  ...
  someAssignedSelected(): boolean {
    return (
      this.assignedEmployees.some((emp) => emp.selected) &&
  ...
  someUnassignedSelected(): boolean {
    return (
      this.unassignedEmployees.some((emp) => emp.selected) &&
    ...
    try {
      ...
      await Promise.all(
        hoursDataList.map((workHoursData) =>
          this.http
            .post(
              `${environment.apiBaseUrl}/save_work_hours.php`,
              workHoursData
            )
            .toPromise()
        )
  ...
  async addIncidentToSelected(isAssigned: boolean) {
    ...
    const incidentOptions = isAssigned
      ? this.assignedIncidents
      : this.unassignedIncidents;
  ...
  async saveIncident(employees: any[], data: any) {
    if (data.incident === "Asistencia sin proyecto") {
        ...
        await Promise.all(
          employees.map(async (employee) => {
            ...
            if (!employee.shift_id) {
              console.error(
                `El empleado ${employee.employee_id} no tiene shift_id asignado.`
              );
              throw new Error(
                `Shift no asignado para el empleado ${employee.employee_id}`
              );
            }
            ...
            if (!shiftInfo) {
              console.error(
                `No se encontró turno para el shift_id ${employee.shift_id}`
              );
              throw new Error(
                `Turno no encontrado para el empleado ${employee.employee_id}`
              );
            }
            ...
            await this.http
              .post(`${environment.apiBaseUrl}/save_incident.php`, incidentData)
              .toPromise();
          })
        );
        await this.toastrService.showSuccess(
          "Se registraron correctamente las horas y la incidencia de asistencia sin proyecto.",
          "Exito"
        );
      } catch (error) {
        console.error("Error al guardar work hours e incidencia:", error);
        await this.toastrService.showError(
          "Ocurrió un error al guardar los registros de asistencia sin proyecto.",
          "Error"
        );
      }
    } else {
      ...
      try {
        ...
        await this.toastrService.showSuccess(
          "Todas las incidencias fueron asignadas correctamente.",
          "Exito"
        );
        ...
        await this.toastrService.showError(
          "Ocurrió un error al asignar las incidencias.",
          "error"
        );
      }
    }
  ...
  async checkIfDayConfirmed(): Promise<boolean> {
    ...
    return new Promise((resolve, reject) => {
      this.http
        .get(
          `${environment.apiBaseUrl}/check_day_confirmed.php?company_id=${this.companyId}&start_date=${start_date}&end_date=${end_date}&week_number=${week_number}&day_of_week=${day_of_week}`
        )
        .subscribe(
          (data: any) => {
            spinner.clear();
            if (data.confirmed) {
              this.toastrService.showError(
                "Este día ya ha sido confirmado. No puedes asignar incidencias ni modificar las horas. Por favor, comunícate con un administrador.",
                "Errorr"
              );
            } else {
          ...
          (error) => {
            spinner.clear();
            console.error(
              "Error al verificar si el día está confirmado:",
              error
            );
            reject(error);
          }
  ...
  // Mostrar selectedWeek.start_date formateado
  get formattedWeekStartDate(): string {
    return this.selectedWeek?.start_date
      ? moment(this.selectedWeek.start_date).format("LL")
      : "No disponible";
  }

  // Mostrar selectedWeek.end_date formateado
  get formattedWeekEndDate(): string {

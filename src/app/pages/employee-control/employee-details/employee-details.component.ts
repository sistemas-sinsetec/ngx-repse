...
export class EmployeeDetailsComponent implements OnInit {
  ...
  constructor(
    ...
    @Inject(NbDialogRef)
    private dialogRef: NbDialogRef<EmployeeDetailsComponent>
  ) {}
  ...
  getEmployeeDetails(employeeId: number) {
    this.http
      .get<{ success: boolean; employee: Empleado; files: EmployeeFile[] }>(
        `${environment.apiBaseUrl}/get_employee_details.php?employee_id=${employeeId}`
      )
      .subscribe((response) => {
        if (response.success) {
  ...
  deleteFile(fileId: number) {
    if (confirm("¿Estás seguro de que deseas eliminar este archivo?")) {
      ...
      this.http
        .post<ApiResponse>(
          `${environment.apiBaseUrl}/delete_employee_file.php`,
          { file_id: fileId }
        )
        .subscribe(
          (response: ApiResponse) => {
            if (response.success) {
              ...
              this.employeeFiles = this.employeeFiles.filter(
                (file) => file.file_id !== fileId
              );
            } else {
              console.error("Error al eliminar el archivo:", response.message);
              alert(
                "Ocurrió un error al intentar eliminar el archivo. Inténtalo de nuevo."
              );
            }
          },
          (error) => {
  ...
  async onFileChange(event: any, fileType: string, fileId?: number) {
    ...
    this.http
      .post(`${environment.apiBaseUrl}/update_upload_files.php`, formData)
      .subscribe(
        async (response) => {
  ...
  triggerFileInput(inputId: string) {
    ...
    if (fileInput) {
      ...
      console.error(
        "No se pudo encontrar el input de archivo con ID:",
        inputId
      );
    }
  }
  ...
  async uploadPhoto(file: File) {
    ...
    this.http
      .post(`${environment.apiBaseUrl}/upload_employee_photo.php`, formData)
      .subscribe(
        async () => {
  ...
  getFileByType(fileType: string): EmployeeFile | null {
    return (
      this.employeeFiles.find((file) => file.file_type === fileType) || null
    );
  }
  ...
  async saveGeneralInfo() {
    ...
    this.http
      .post(`${environment.apiBaseUrl}/update_general_info.php`, generalInfo)
      .subscribe(
        async (response: any) => {
          await loading.dismiss();
          if (response.success) {
            this.toastrService.showSuccess(
              "Información general actualizada con éxito",
              "Éxito"
            );
          } else {
        ...
        async (error) => {
          await loading.dismiss();
          this.toastrService.showError(
            "Error al actualizar la información general",
            "Error"
          );
  ...
  validateForm(): boolean {
    if (!this.employee.first_name.match(/^[a-zA-ZáéíóúÁÉÍÓÚñÑ\s]+$/))
      return false;
    if (!this.employee.last_name.match(/^[a-zA-ZáéíóúÁÉÍÓÚñÑ\s]+$/))
      return false;
    if (!this.employee.middle_name.match(/^[a-zA-ZáéíóúÁÉÍÓÚñÑ\s]+$/))
      return false;
    ...
    if (this.employee.clabe && !this.employee.clabe.match(/^\d{18}$/))
      return false;
  ...
  async saveFinancialInfo() {
    ...
    this.http
      .post(
        `${environment.apiBaseUrl}/update_financial_info.php`,
        financialInfo
      )
      .subscribe(
        async () => {
          await loading.dismiss();
          this.toastrService.showSuccess(
            "Información financiera actualizada con éxito",
            "Éxito"
          );
        },
        async (error) => {
  ...
  async saveWorkInfo() {
    ...
    this.http
      .post(`${environment.apiBaseUrl}/update_work_info.php`, workInfo)
      .subscribe(
        async () => {
          await loading.dismiss();
          this.toastrService.showSuccess(
            "Información laboral actualizada con éxito",
            "Éxito"
          );
        },
        async (error) => {
  ...
  async saveEmergencyContact() {
    ...
    this.http
      .post(
        `${environment.apiBaseUrl}/update_emergency_contact.php`,
  ...
  getMaritalStatuses() {
    this.http
      .get<any[]>(`${environment.apiBaseUrl}/get_marital_statuses.php`)
      .subscribe(
        (response) => {
  ...
  async downloadEmployeeData() {
    ...
    if (!folder) {
      console.error(
        `No se pudo crear la carpeta para el empleado ${this.employee.full_name}`
      );
      return;
    }
    ...
    Estado en el IMSS: ${
      this.employee.request_id &&
      this.employee.folio_number_imss &&
      this.employee.lot_number_imss
        ? "Activo en el IMSS"
        : "No activo en el IMSS"
    }
    ...
    Estado Civil: ${
      this.getMaritalStatusName(this.employee.marital_status_id) || ""
    }
  ...
  `;
    ...
    if (!archivosFolder) {
      console.error(
        `No se pudo crear la subcarpeta "archivos" para el empleado ${this.employee.full_name}`
      );
      return;
    }
  ...
  async getFileBlob(filePath: string): Promise<Blob> {
    try {
      ...
      const response = await fetch(
        `${environment.apiBaseUrl}/download.php?file_path=${encodedFilePath}`
      );

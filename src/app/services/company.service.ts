...
export class CompanyService {
  ...
  loadMappedPrincipalCompanies(): void {
    const mappedPrincipalCompaniesString = localStorage.getItem(
      "mappedPrincipalCompanies"
    );
  ...
  loadCompanyLogo(companyId: string): Promise<string> {
    return this.http
      .get(
        `${environment.apiBaseUrl}/getCompanyLogo.php?companyId=${companyId}`
      )
      .toPromise()
      .then((response: any) => {
  ...
  updateLogo(newLogoUrl: string): void {
    if (this.selectedCompany) {
      ...
      console.log(
        "Logo actualizado en el servicio y en localStorage:",
        this.selectedCompany
      );
    }
  }
  ...
  async selectAndLoadCompany(company: any): Promise<void> {
    ...
    try {
      const response: any = await this.loadNonPrincipalCompanies(
        company.id
      ).toPromise();

      if (response && response.success) {
        const mappedCompanies = response.nonPrincipalCompanies.map(
          (c: any) => ({
            id: c.idAssociation,
            name: c.nameCompany,
            role: c.roleInCompany,
